...
const NATIVE_TOKEN = {
  ...
  wrappedAddress: '0x0BE9e53fd7EDaC9F859882AfdDa116645287C629', // WFUSE
};
...
const fuse: ChainConfig = {
  ...
  transactionApi: {
    ...
    buildUrl: (address, limit, offset) => {
      ...
      return `https://explorer.fuse.io/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  txUrl: (hash) => `https://explorer.fuse.io/tx/${hash}`,
  addressUrl: (addr) => `https://explorer.fuse.io/address/${addr}`,

  theme: {
    primary: '#B4F9BA',
    gradient: 'linear-gradient(135deg, #B4F9BA 0%, #1E4D3D 100%)',
  },
};
