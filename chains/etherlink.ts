// chains/etherlink.ts
// Etherlink chain configuration (Tezos L2)
...
const NATIVE_TOKEN = {
  ...
  decimals: 18,
  coingeckoId: 'tezos',
  wrappedAddress: '0xc9B53AB2679f573e480d01e0f49e2B5CFB7a3EAB', // WXTZ
};
...
const etherlink: ChainConfig = {
  ...
  transactionApi: {
    ...
    buildUrl: (address, limit, offset) => {
      ...
      return `https://explorer.etherlink.com/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  txUrl: (hash) => `https://explorer.etherlink.com/tx/${hash}`,
  addressUrl: (addr) => `https://explorer.etherlink.com/address/${addr}`,

  theme: {
    primary: '#2C7DF7',
    gradient: 'linear-gradient(135deg, #2C7DF7 0%, #1A5BC4 100%)',
  },
};
