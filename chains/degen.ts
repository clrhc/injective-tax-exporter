// chains/degen.ts
// Degen L3 chain configuration (Base L3)
...
const NATIVE_TOKEN = {
  ...
  wrappedAddress: '0xEb54dACB4C2ccb64F8074eceEa33b5eBb38E5387', // WDEGEN
};
...
const degen: ChainConfig = {
  ...
  transactionApi: {
    ...
    buildUrl: (address, limit, offset) => {
      ...
      return `https://explorer.degen.tips/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  buildTokenMap: (tokens: any) => {
    ...
    for (const t of tokenList) {
      if (t.chainId === 666666666 || !t.chainId) {
  ...
  txUrl: (hash) => `https://explorer.degen.tips/tx/${hash}`,
  addressUrl: (addr) => `https://explorer.degen.tips/address/${addr}`,

  theme: {
    primary: '#A36EFD',
    gradient: 'linear-gradient(135deg, #A36EFD 0%, #7B4FBC 100%)',
  },
};
