...
const NATIVE_TOKEN = {
  ...
  decimals: 18,
  coingeckoId: 'astar',
  wrappedAddress: '0xAeaaf0e2c81Af264101B9129C00F4440cCF0F720', // WASTR
};
...
const astar: ChainConfig = {
  ...
  transactionApi: {
    baseUrl: 'https://astar.blockscout.com/api',
    buildUrl: (address, limit, offset) => {
      const page = Math.floor(offset / limit) + 1;
      return `https://astar.blockscout.com/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  txUrl: (hash) => `https://astar.blockscout.com/tx/${hash}`,
  addressUrl: (addr) => `https://astar.blockscout.com/address/${addr}`,

  theme: {
    primary: '#0070EB',
    gradient: 'linear-gradient(135deg, #0070EB 0%, #E6007A 100%)',
  },
};
