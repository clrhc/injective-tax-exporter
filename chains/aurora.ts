...
const NATIVE_TOKEN = {
  ...
  wrappedAddress: '0xC9BdeEd33CD01541e1eeD10f90519d2C06Fe3feB', // WETH on Aurora
};
...
const aurora: ChainConfig = {
  ...
  transactionApi: {
    baseUrl: 'https://explorer.aurora.dev/api',
    buildUrl: (address, limit, offset) => {
      const page = Math.floor(offset / limit) + 1;
      return `https://explorer.aurora.dev/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  buildTokenMap: (tokens: any) => {
    ...
    for (const t of tokenList) {
      if (t.chainId === 1313161554 || !t.chainId) {
  ...
  txUrl: (hash) => `https://explorer.aurora.dev/tx/${hash}`,
  addressUrl: (addr) => `https://explorer.aurora.dev/address/${addr}`,

  theme: {
    primary: '#70D44B',
    gradient: 'linear-gradient(135deg, #70D44B 0%, #3D8C2F 100%)',
  },
};
