// chains/etc.ts
// Ethereum Classic chain configuration
...
const NATIVE_TOKEN = {
  ...
  wrappedAddress: '0x82A618305706B14e7bcf2592D4B9324A366b6dAd', // WETC
};
...
const etc: ChainConfig = {
  ...
  transactionApi: {
    baseUrl: 'https://etc.blockscout.com/api',
    buildUrl: (address, limit, offset) => {
      const page = Math.floor(offset / limit) + 1;
      return `https://etc.blockscout.com/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  txUrl: (hash) => `https://etc.blockscout.com/tx/${hash}`,
  addressUrl: (addr) => `https://etc.blockscout.com/address/${addr}`,
