// chains/bob.ts
// BOB (Build on Bitcoin) chain configuration
...
const NATIVE_TOKEN = {
  ...
  wrappedAddress: '0x4200000000000000000000000000000000000006', // WETH on BOB
};
...
const bob: ChainConfig = {
  ...
  transactionApi: {
    baseUrl: 'https://explorer.gobob.xyz/api',
    buildUrl: (address, limit, offset) => {
      const page = Math.floor(offset / limit) + 1;
      return `https://explorer.gobob.xyz/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  txUrl: (hash) => `https://explorer.gobob.xyz/tx/${hash}`,
  addressUrl: (addr) => `https://explorer.gobob.xyz/address/${addr}`,

  theme: {
    primary: '#F7931A',
    gradient: 'linear-gradient(135deg, #F7931A 0%, #FF6B00 100%)',
  },
};
