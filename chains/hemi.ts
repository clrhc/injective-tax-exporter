// chains/hemi.ts
// Hemi chain configuration (Bitcoin + Ethereum layer)
...
const NATIVE_TOKEN = {
  ...
  wrappedAddress: '0x4200000000000000000000000000000000000006', // WETH on Hemi
};
...
const hemi: ChainConfig = {
  ...
  transactionApi: {
    ...
    buildUrl: (address, limit, offset) => {
      ...
      return `https://explorer.hemi.xyz/api?module=account&action=txlist&address=${address}&page=${page}&offset=${limit}&sort=desc`;
    },
  ...
  txUrl: (hash) => `https://explorer.hemi.xyz/tx/${hash}`,
  addressUrl: (addr) => `https://explorer.hemi.xyz/address/${addr}`,
